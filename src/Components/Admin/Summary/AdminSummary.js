...
const ContentBox = ({ title, content }) => {
  return (
    <Paper elevation={3} sx={{ mb: 2, width: "100%" }}>
      <Box
        sx={{
          display: "flex",
...
function AdminSummary({ user }) {
  ...
  const availabilityForWeekday = (weekday) => {
    return availabilitySummary.filter(
      (availability) => availability.day_of_week === weekday
    );
  };
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const trainedResponse = await api.trainedSummaryWithID(
          user.employee_id
        );
        setTrainedSummary(trainedResponse.data);

        const availabilityResponse = await api.availabilitySummaryWithID(
          user.employee_id
        );
        setAvailabilitySummary(availabilityResponse.data);

        const requestsResponse = await api.requestsSummaryWithID(
          user.employee_id
        );
        setRequestsSummary(requestsResponse.data);

        const nextShiftResponse = await api.nextShiftForEmployee(
          user.employee_id
        );
        setNextShift(nextShiftResponse.data);

        const lastPunchResponse = await api.lastPunchForEmployee(
          user.employee_id
        );
        setLastPunch(lastPunchResponse.data);

        const availabilityTimeOffPendingCountResponse =
          await api.availabilityTimeOffPendingCount();
        setAvailabilityTimeOffPendingCount(
          availabilityTimeOffPendingCountResponse.data
        );
            ...
            content={
              ...
              [
                ...
                <Box key={item} width="100%">
                  <Typography variant="h6">{capitalize(item)}</Typography>
                  <Typography variant="body">
                    {availabilityForWeekday(item) &&
                    availabilityForWeekday(item).length > 0
                      ? `${DateHelper.formatTime(
                          availabilityForWeekday(item)[0].start_time
                        )} - ${DateHelper.formatTime(
                          availabilityForWeekday(item)[0].end_time
                        )}`
                      : "OFF"}
                  </Typography>
                  {item !== "Saturday" && (
                  ...
                  >
                    ...
                    <Typography variant="h4">
                      {availabilityTimeOffPendingCount &&
                        availabilityTimeOffPendingCount.length > 0 &&
              ...
              content={
                lastPunch.length > 0 ? (
                  <Box width="100%" display="flex" flexDirection="column">
                    <Typography variant="h6">
                      {`${DateHelper.shortDateFormat(
                        DateHelper.textToDate(lastPunch[0].punchin)
                      )} at ${DateHelper.friendlyTimeFormat(
                        DateHelper.textToDate(lastPunch[0].punchin)
                      )}`}
              ...
              content={
                nextShift.length > 0 ? (
                  <Box width="100%" display="flex" flexDirection="column">
                    <Typography variant="h6">
                      {`${DateHelper.shortDateFormat(
                        DateHelper.textToDate(nextShift[0].start_time)
                      )} from ${ScheduleHelper.getTimesForShift(nextShift[0])}`}
                    </Typography>
                    <Typography variant="body" mb={0.5}>
                  ...
                  ? requestsSummary
                      ...
                      .map((item, index) => (
                        <Box key={index} width="100%">
                          <Typography variant="h6">
                            {`${DateHelper.shortDateFormat(
                              DateHelper.textToDate(item.start_time)
                            )} - ${DateHelper.shortDateFormat(
                              DateHelper.textToDate(item.end_time)
                            )}`}
                          </Typography>
